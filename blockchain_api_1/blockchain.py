...
class BlockChain(object):
    ...
    def broadcast_transaction(self, transaction):
        ...
        for url in OTHER_API_LIST:
            destination_url = url + "/receive_transaction"
            # print(destination_url)
            # type(transaction)
            # type(transaction_dict)
            res = requests.post(destination_url, json.dumps(transaction_dict))
            print(res.json())

    def broadcast_chain(self, chain):
        for url in OTHER_API_LIST:
            destination_url = url + "/receive_chain"
            res = requests.post(destination_url, json.dumps(chain))
            print(res.json())
